  ...
  */
	...
	public void searchInfo() {
		...
		if(selectNum == 1) {
			...
			int idx = searchIdx(searchName);
				...
				if(idx > -1) {
					System.out.println("이름: "+pbs[idx].getName());
					System.out.println("전화번호: "+pbs[idx].getPhoneNum());
				}
				else {
	...
	public void modifyInfo() {
		...
		System.out.println("전화번호 입력");
		 ...
		 for(int i=0; i<pbs.length; i++) {
			 if(searchName.equals(pbs[i].getName()) 
					 && searchNum.equals(pbs[i].getPhoneNum())) {
				...
				if(selectModify == 1) {
					...
					System.out.println(pbs[0].getName());
					 break;
				}
					...
					String modifyNum = scanner.next();
					 pbs[i].setPhoneNum(modifyNum);
						System.out.println(pbs[0].getPhoneNum());
						 break; 
	...
	int idx = -1;
			...
			for(int i=0; i<pbs.length; i++) {
				if(pbs[i] != null) {
				if(search.equals(pbs[i].getName())) {
					idx=i;
					break;
				}
				}
